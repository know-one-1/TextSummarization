...
###############For Discriminator ##########Added by pm 

@curry
def prepro_fn_discrim(max_src_len, batch):
    def prepro_one(sample):
        source_sents, target_class = sample
        tokenized_sents = tokenize(max_src_len, source_sents)
        return tokenized_sents, target_class
    batch = list(map(prepro_one, batch))
    return batch

@curry
def batchify_fn_discrim_conv(pad, data, cuda=True):
    source_lists, targets = tuple(map(list, unzip(data)))
    src_nums = list(map(len, source_lists))
    sources=pad_batch_tensorize_2(source_lists,pad=pad, cuda=cuda)
    tensor_type = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    target = tensor_type(list(targets))
    fw_args = torch.cuda.LongTensor([sources]) if cuda else torch.LongTensor([sources])                                                                                                                                                                                                                                                                                                                                                   
    loss_args = (target, )
    return fw_args, loss_args

@curry
def convert_batch_discrim_conv(unk, word2id, batch):
    def convert_one(sample):
        source_sents, target_class = sample
        id_sents = conver2id(unk, word2id, source_sents)
        binary_class = [0] * 2
        binary_class[target_class]=1
        return id_sents, binary_class
    batch = list(map(convert_one, batch))
    return batch


@curry
def pad_batch_tensorize_2(inputs, pad, cuda=True):
    tensor_type = torch.cuda.LongTensor if cuda else torch.LongTensor
    batch_size = len(inputs)
    max_len = max(len(ids[0]) for ids in inputs)
    tensor_shape = (batch_size, max_len)
    tensor=[[pad]*max_len]*batch_size
    for i, ids in enumerate(inputs):
        tensor[i]  = ids[0] +tensor[i][:-len(ids[0])]
        #print(tensor[i],ids)
    return tensor

def coll_fn_discrim(data):
    def is_good_data(d):
        """ make sure data is not empty"""
        source_sents, target_class = d
        return source_sents and True if target_class or target_class==0 else False
    batch = list(filter(is_good_data, data))
    assert all(map(is_good_data, batch))
    return batch
...
@curry
def copy_prepro_fn_discrim(max_src_len, batch):
    def prepro_one(sample):
        source_sents,  extract, target_class = sample
        tokenized_sents = tokenize(250, source_sents)
        tokenized_extrts = tokenize(max_src_len, extract)
        return tokenized_sents, tokenized_extrts, target_class
    batch = list(map(prepro_one, batch))
    return batch

@curry
def copy_batchify_fn_discrim_conv(pad, data, cuda=True):
    source_lists, interm = map(list, unzip(data))
    extract_lists, targets = map(list, unzip(interm))
    sources=copy_pad_batch_tensorize_2(source_lists,pad=pad, cuda=cuda)
    extracts=copy_pad_batch_tensorize_2(extract_lists,pad=pad, cuda=cuda)
    tensor_type = torch.cuda.FloatTensor if cuda else torch.FloatTensor
    target = tensor_type(list(targets))
    fw_arg1 = torch.cuda.LongTensor([sources]) if cuda else torch.LongTensor([sources])                                                                                                                                                                                                                                                                                                                                                   
    fw_arg2 = torch.cuda.LongTensor([extracts]) if cuda else torch.LongTensor([extracts])
    fw_args = fw_arg1, fw_arg2
    loss_args = (target, )
    return fw_args, loss_args

@curry
def copy_convert_batch_discrim_conv(unk, word2id, batch):
    def convert_one(sample):
        source_sents, extracts, target_class = sample
        id_sents = conver2id(unk, word2id, source_sents)
        id_extrts = conver2id(unk, word2id, extracts)
        binary_class = [0] * 2
        binary_class[target_class]=1
        return id_sents, (id_extrts, binary_class)
    batch = list(map(convert_one, batch))
    return batch


@curry
def copy_pad_batch_tensorize_2(inputs, pad, cuda=True):
    tensor_type = torch.cuda.LongTensor if cuda else torch.LongTensor
    batch_size = len(inputs)
    max_len = max(len(ids[0]) for ids in inputs)
    tensor_shape = (batch_size, max_len)
...
def copy_coll_fn_discrim(data):
    def is_good_data(d):
        """ make sure data is not empty"""
        source_sents, extract ,target_class = d
        return source_sents and extract and True if target_class or target_class==0 else False
    batch = list(filter(is_good_data, data))
    assert all(map(is_good_data, batch))
    return batch
