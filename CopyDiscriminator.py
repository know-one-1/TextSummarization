...
from data.batcher import coll_fn_extract, copy_prepro_fn_discrim
from data.batcher import copy_batchify_fn_discrim_conv
from data.batcher import copy_convert_batch_discrim_conv
...
class CopyGetDataset1(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        article,abstract, target_class = [" ".join(js_data['article'])],[" ".join(js_data['abstract'])] , 1
        return article, abstract, target_class


class CopyGetDataset2(DiscrimDataset):
...
class GetArticle(CnnDmDataset):
    ...
    def __getitem__(self, i):
        js_data = super().__getitem__(i)
        return [" ".join(js_data['article'])]

class Concat(Dataset):
    def __init__(self, datasets):
        self.d1,self.d2 = datasets

    def __getitem__(self, i):
        a,b=self.d2.__getitem__(i)
        return self.d1.__getitem__(i),a,b

    def __len__(self):
        return min(self.d1.__len__(),self.d2.__len__() )


def build_batchers(net_type, word2id, cuda, debug):
    assert net_type in ['cnn']
    prepro = copy_prepro_fn_discrim(args.max_word)
    def sort_key(sample):
        src_sents, _, _ = sample
        return len(src_sents)
    batchify_fn = copy_batchify_fn_discrim_conv 
    convert_batch = copy_convert_batch_discrim_conv 
                     
    batchify = compose(batchify_fn(PAD, cuda=cuda),
                       convert_batch(UNK, word2id))
    
    trainset1=CopyGetDataset1('train')
    trainset2=Concat([GetArticle('train'),CopyGetDataset2('train')])
    ...
    valset1=CopyGetDataset1('val')
    valset2=Concat([GetArticle('val'),CopyGetDataset2('val')])
