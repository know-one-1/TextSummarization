...
def decode(save_path, model_dir, split, batch_size,
    ...
    os.makedirs(join(save_path, split))
    dec_log = {}
    ...
    with open(join(save_path, 'log_{}.json'.format(split)), 'w') as f:
    ...
    with torch.no_grad():
        for i_debug, raw_article_batch in enumerate(loader):
            ...
            for j, n in ext_inds:
                ...
                dec={}
                dec['abstract']=make_html_safe('\n'.join(decoded_sents)).split('\n')
                dec['class']=0
                dec_serial=json.dumps(dec,indent=4)
...
if __name__ == '__main__':
    ...
    for data_split in ['test','train','val']:
